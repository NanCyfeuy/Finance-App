...
/// Layar edit dompet — sama strukturnya dengan TambahDompet tapi pre-filled.
class EditDompetScreen extends StatefulWidget {
  final WalletModel wallet;
  const EditDompetScreen({super.key, required this.wallet});

  @override
  State<EditDompetScreen> createState() => _EditDompetScreenState();
}

class _EditDompetScreenState extends State<EditDompetScreen> {
  ...
  void initState() {
    super.initState();
    _namaCtrl = TextEditingController(text: widget.wallet.nama);
    _saldoCtrl =
        TextEditingController(text: widget.wallet.saldo.toString());
    _tipe = widget.wallet.tipe;
  }
  ...
  void _simpan() {
    ...
    final saldo =
        int.tryParse(_saldoCtrl.text.replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;
        ...
        child: ListView(
          ...
          children: [
            ...
            _buildTextField(
              ...
              validator: (v) =>
                  (v == null || v.isEmpty) ? 'Saldo wajib diisi' : null,
            ),
            const SizedBox(height: 16),
              ...
              children: _tipeOptions.map((opt) {
                ...
                return Padding(
                  ...
                  child: GestureDetector(
                    onTap: () =>
                        setState(() => _tipe = opt['value'] as String),
